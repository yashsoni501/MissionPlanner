...
{
    public partial class FlightPlanner2 : MyUserControl, IDeactivate, IActivate
    {
        public FlightPlanner2()
        {
            InitializeComponent();
            Init();
        }
        ...
        public GMapOverlay destinationOverlay;
        private bool grid;
        ...
        public GMapMarker destMarker = new GMarkerGoogle(new PointLatLng(0.0, 0.0), GMarkerGoogleType.none);
        ...
        private ComponentResourceManager rm = new ComponentResourceManager(typeof(FlightPlanner2));
        ...
        public void Init()
        {
            ...
            MainMap.Overlays.Add(destinationOverlay);
        ...
        public static FlightPlanner2 instance { get; set; }
        ...
        public void AddWPToMap(double lat, double lng, int alt, MAVLink.MAV_CMD wpType, int del = 0)
        {
            if (polygongridmode)
            {
                addPolygonPointToolStripMenuItem_Click(null, null);
            ...
            */
            //{
            Commands.Rows[selectedrow].Cells[Command.Index].Value = ((ushort)wpType).ToString();
            ChangeColumnHeader(MAVLink.MAV_CMD.WAYPOINT.ToString());
            //}
            if (del>0)
            {
                // Commands.Columns[Param4.Index].HeaderText = "Delay";
                DataGridViewTextBoxCell cell = Commands.Rows[selectedrow].Cells[Param4.Index] as DataGridViewTextBoxCell;
                cell.Value = del;
        ...
        public void FlightPlanner2_FormClosing(object sender, FormClosingEventArgs e)
        {
            timer1.Stop();
        }

        public void FlightPlanner2_Load(object sender, EventArgs e)
        {
            quickadd = true;
...
Column 1: Field type (RALLY is the only one at the moment -- may have RALLY_LAND in the future)
 ...
 Column 7: Flags (just 0 for now, also future use).
        ...
        public void BUT_reset_HOME_Click(object sender, EventArgs e)
        {
            ...
            {
                CustomMessageBox.Show(
                    "Drone not connnected. Please connect the drone and click again on \"Update Home\" button");
            }
        }
        ...
        private void BUT_mission_oneway_Click(object sender, EventArgs e)
        {
            ...
            AddWPToMap(dest.lat, dest.lng, zero, MAVLink.MAV_CMD.LAND);
            AddWPToMap(dest.lat, dest.lng, zero, MAVLink.MAV_CMD.DELAY, 5);
            AddWPToMap(dest.lat, dest.lng, zero, MAVLink.MAV_CMD.DO_GRIPPER);
            AddWPToMap(dest.lat, dest.lng, zero, MAVLink.MAV_CMD.DELAY, 5);
        ...
        private void BUT_mission_return_Click(object sender, EventArgs e)
        {
            ...
            AddWPToMap(dest.lat, dest.lng, zero, MAVLink.MAV_CMD.DO_GRIPPER);
            AddWPToMap(dest.lat, dest.lng, zero, MAVLink.MAV_CMD.DELAY, 5);
            AddWPToMap(dest.lat, dest.lng, missionAlt, MAVLink.MAV_CMD.TAKEOFF);
            AddWPToMap(home.lat, home.lng, missionAlt);
            AddWPToMap(home.lat, home.lng, zero, MAVLink.MAV_CMD.LAND);

            saveRallyPointsToolStripMenuItem_Click(null, null);
            BUT_write_Click(null, null);
        }

        private void DestLat_TextChanged(object sender, EventArgs e)
        {
            if (DestLat.Text == "")
            {
                dest.lat = 0;
            ...
            dest.lat = double.Parse(DestLat.Text);
        }

        private void DestLong_TextChanged(object sender, EventArgs e)
        {
            if (DestLong.Text == "")
            {
                dest.lng = 0;
        ...
        {
            destMarker = new GMarkerGoogle(new PointLatLng(dest.lat, dest.lng), GMarkerGoogleType.blue);
            destinationOverlay.Markers.Clear();
            destinationOverlay.Markers.Add(destMarker);
            MainMap.UpdateMarkerLocalPosition(destinationOverlay.Markers[0]);
        }
    }
}
